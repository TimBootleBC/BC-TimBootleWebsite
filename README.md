...
The reading list lets the owner update reading progress, favorites, and the
**Read Again?** Yes/No choice. It stores edits in the visitor's browser until
**Publish changes** is selected. That button opens a prefilled GitHub issue. When the issue is
...
closes the issue. The JSON file—not private-window browser storage—is the shared
source of truth, so published choices appear in normal and InPrivate windows
after GitHub Pages deploys. Change both the `REPOSITORY` constant in `reading-list.html`
