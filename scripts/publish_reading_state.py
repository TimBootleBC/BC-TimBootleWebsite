...
def main() -> None:
    ...
    for book_id, book_state in sorted(books.items()):
        ...
        if not isinstance(book_state, dict) or not book_state or not set(book_state) <= {"status", "favorite", "readAgain"}:
        ...
        if "readAgain" in book_state:
            if not isinstance(book_state["readAgain"], bool):
                fail(f"readAgain must be true or false for {book_id}")
            clean["readAgain"] = book_state["readAgain"]
